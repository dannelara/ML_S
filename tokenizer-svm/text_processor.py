...
def tokenize_lemmatize(text):
    text = text.replace(".", "")
    return [token.lemma_ for token in nlp_sv(text)]

def remove_stop_words(words, lang):
    stopwords = STOPWORDS_SE if lang == "sv" else STOPWORDS_EN
    return [word for word in words if word.lower() not in stopwords]

def process_text(text, algo):
    if "lang" not in process_text.__dict__:
        process_text.lang = detect_langs(text)[0].lang
    language = process_text.lang
    words = None
    if algo == "lem":
...
def process_data_file(filename):
    ...
    processed_data = [process_text(name, "stem") for name in names]
